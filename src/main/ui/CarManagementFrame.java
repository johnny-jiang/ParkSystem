 ...
 * @version: 0.1
 */
public class CarManagementFrame extends javax.swing.JFrame {
	...
	public CarManagementFrame() {
		...
		this.cManage = new CarManagement();
		initComponents();
	}

	public CarManagementFrame(CarManagement cm) {
		/* Set the Metal look and feel */
		try {
			for (UIManager.LookAndFeelInfo info : UIManager
					.getInstalledLookAndFeels()) {
	...
	private void initComponents() {
		...
		jTableData.setModel(new DefaultTableModel(cManage.getCellData(),
				cManage.getColumnNames()) {
	...
	private void jButtonAddActionPerformed(java.awt.event.ActionEvent evt) {
		...
		DefaultTableModel tableModel = (DefaultTableModel) this.jTableData
				.getModel();
		tableModel.addRow(CarDataBase.getNewData());
	}
	...
	private void jButtonSaveActionPerformed(java.awt.event.ActionEvent evt) {
		...
		this.cManage.clearList();
		for (int i = 0; i < rowCount; i++) {
			if(tableModel.getValueAt(i, 1).equals(""))
				continue;
			CarDataBase cDat = new CarDataBase();
			cDat.setOwnerID((Integer) tableModel.getValueAt(i, 0));
			cDat.setLicense((String) tableModel.getValueAt(i, 1));
			cDat.setName((String) tableModel.getValueAt(i, 2));
			cManage.addCar(cDat);
		}
		this.cManage.saveCarDataToFile();
	}
